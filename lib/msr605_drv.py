...
def msr_init(dev_path):
    try:
        res = serial.Serial(dev_path, 9600, 8, serial.PARITY_NONE, timeout=0.01)
    except:
        res = None
...
def msr_reset(dev_ptr):
    try:
        dev_ptr.write(b"\x1B"+b"\x61") # escape_code = \x1B
        time.sleep(0.1)
...
def execute_waitresult(command, dev_ptr, timeout=10):
    # execute
    dev_ptr.flushInput()
    dev_ptr.write(b"\x1B"+command) # escape_code = \x1B
    time.sleep(0.1)
...
def set_coercivity(coercivity,dev_ptr):
    if coercivity == 'hico':
        status, res, data = execute_waitresult(b"\x78", dev_ptr) # x
    elif coercivity == 'loco':
        status, res, data = execute_waitresult(b"\x79", dev_ptr) # y

    if status != 48: # 48d = "0"
        print((" [-] set_coercivity() error: %c" % status))
        #raise Exception(" [-] set_coercivity() error: %c" % status)
        return False
    print(" [+] set coercivity: " + coercivity)

def set_bpc(bpc1, bpc2, bpc3,dev_ptr):
    status, res, data = execute_waitresult(("\x6F"+chr(bpc1)+chr(bpc2)+chr(bpc3)).encode(), dev_ptr) # \x6F: o
    if status != 48: # 48d = "0"
        print((" [-] set_bpc() error: %c" % status))
        #raise Exception(" [-] set_bpc() error: %c" % status)
        return False
    print(" [+] set bpc: %d %d %d" % (bpc1, bpc2, bpc3))

def read_raw_tracks(dev_ptr):
    status, res, data = execute_waitresult(b"\x6D",dev_ptr) # m : read raw data
    #data = map_7bits[0]
    if status != 48: # 48d = "0"
...
def read_iso_tracks(dev_ptr):
    status, res, data = execute_waitresult(b"\x72",dev_ptr) # r
    if status != 48: # 48d = "0"
...
def decode_rawdatablock(data):
    ...
    #print(data)
    strip1_start = 4
    strip1_end = strip1_start + 1 + data[strip1_start] # first byte is length
    ...
    # trailer
    if data[strip3_end:-2] != b"\x3F"+b"\x1C":
...
def encode_rawdatablock(strip1, strip2, strip3):
    ...
    #return b"\x1b\x73\x1b\x01"+chr(len(strip1))+strip1+b"\x1b\x02"+chr(len(strip2))+strip2+b"\x1b\x03"+chr(len(strip3))+strip3+b"\x3F\x1C"

    return b"\x1b\x73\x1b\x01"+chr(len(strip1)).encode()+strip1.encode()+b"\x1b\x02"+chr(len(strip2)).encode()+strip2.encode()+b"\x1b\x03"+chr(len(strip3)).encode()+strip3.encode()+b"\x3F\x1C"

def encode_isodatablock(strip1, strip2, strip3):
    # use empty string if you don't want to set a given strip
    return b"\x1b\x73\x1b\x01"+strip1.encode()+b"\x1b\x02"+strip2.encode()+b"\x1b\x03"+strip3.encode()+b"\x3F\x1C"

#def erase_tracks(dev_ptr,trackSelect):
def erase_tracks(dev_ptr,t1=False, t2=False, t3=False):
#    mask = 0
#    if t1: mask |=1
#    if t2: mask |=2
#    if t3: mask |=4
    #if not(trackSelect >= 0 and trackSelect <=7 and trackSelect != 1):
    #    print " [-] Trak selection is invalid"
    #    return False
    #mask = trackSelect
    if t1==True and t2==True and t3==True:
        mask = b"\x07"
    if t1==False and t2==True and t3==True:
        mask = b"\x06"
    if t1==True and t2==False and t3==True:
        mask = b"\x05"
    if t1==True and t2==True and t3==False:
        mask = b"\x03"
    if t1==False and t2==False and t3==True:
        mask = b"\x04"
    if t1==False and t2==True and t3==False:
        mask = b"\x02"
    if t1==True and t2==False and t3==False:
        mask = b"\x01"

    status, res, data = execute_waitresult(b"\x63" + mask,dev_ptr)
    #status, _, _ = execute_waitresult("\x63"+chr(int(mask)),dev_ptr)
    #status, _, _ = execute_waitresult("\x63"+str(mask),dev_ptr)
    if status != 48: # 48d = "0"
...
def set_leadingzero(self, track13, track2):
    status, res, data = execute_waitresult(b"\x7A"+track13+track2)
    if status != 48: # 48d = "0"
...
def set_bpi(bpi1, bpi2, bpi3,dev_ptr):
    modes = []
    if bpi1=="210":
        modes.append(b"\xA1")    # 210bpi
    elif bpi1=="75":
        modes.append(b"\xA0")    # 75bpi
    if bpi2=="210":
        modes.append(b"\xD2")    # 210bpi
    elif bpi2=="75":
        modes.append(b"\x4B")    # 75bpi

    if bpi3=="210":
        modes.append(b"\xC1")    # 210bpi
    elif bpi3=="75":
        modes.append(b"\xC0")    # 75bpi

    for m in modes:
        status, res, data = execute_waitresult(b"\x62"+m,dev_ptr)
        if status != 48: # 48d = "0"
...
def get_hico_loco_status(dev_ptr):
    result = dev_ptr.write(b"\x1B"+b"\x64") # escape_code = \x1B
    time.sleep(0.1)
    if result == 'H':
        return 'hico'
    elif result == 'L':
...
def do_ram_test(dev_ptr):
    result = dev_ptr.write(b"\x1B"+b"\x86") # escape_code = \x1B
    time.sleep(0.1)
    if result != "0":
       return 'KO'
    elif result == "0":
...
def do_communication_test(dev_ptr):
    result = dev_ptr.write(b"\x1B"+b"\x65") # escape_code = \x1B
    time.sleep(0.1)
    if result == "y":
        return 'OK'
    elif result != "y":
...
def do_sensor_test():
    status, res, data = execute_waitresult(b"\x86")
    if status != 48: # 48d = "0"
...
def play_all_led_off(dev_ptr):
    dev_ptr.write(b"\x1B"+b"\x81")
    print(" [+] all led off")

def play_all_led_on(dev_ptr):
    dev_ptr.write(b"\x1B"+b"\x82")
    print(" [+] all led on")

def play_green_led_on(dev_ptr):
    dev_ptr.write(b"\x1B"+b"\x83")
    print(" [+] green led on")

def play_yellow_led_on(dev_ptr):
    dev_ptr.write(b"\x1B"+b"\x84")
    print(" [+] yellow led on")

def play_red_led_on(dev_ptr):
    dev_ptr.write(b"\x1B"+b"\x85")
    print(" [+] red led on")
